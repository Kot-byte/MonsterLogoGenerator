...
export const initPersonalizationList = () => {
    let defaultPersonalizeType;
    
    for (let key in activeLogosDataItem.personalize) {
        if (key !== 'none') {
            defaultPersonalizeType = key;
            break;
        }
    }
    changePersonalizationType(defaultPersonalizeType, activeLogosDataItem);
    personalizationList.append(generatePersonalizationList());

    targetPersonalizeBtn = personalizationList.querySelectorAll('.personalize-btn')[1];
